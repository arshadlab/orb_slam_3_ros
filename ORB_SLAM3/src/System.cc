...
CameraInfo System::defaultParams;

Verbose::eLevel Verbose::th = Verbose::VERBOSITY_NORMAL;

System::System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor,
               const bool bUseViewer, const int initFr, const string &strSequence, CameraInfo parameters):
    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    if(!node.empty() && node.isString() && node.string() == "1.0"){
        settings_ = new Settings(strSettingsFile,mSensor, parameters);
    ...
    else
    {
        ...
        {
            cout << "Error loading the file; Continuing with empty map" << endl;
            mpAtlas = new Atlas(0);
        }
        //mpKeyFrameDatabase = new KeyFrameDatabase(*mpVocabulary);
...
Sophus::SE3f System::GetCurrentPosition () {
