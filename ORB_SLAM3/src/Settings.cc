...
namespace ORB_SLAM3 {
    ...
    Settings::Settings(const std::string &configFile, const int& sensor, const CameraInfo &parameters) :
    bNeedToUndistort_(false), bNeedToRectify_(false), bNeedToResize1_(false), bNeedToResize2_(false) {
        ...
        //Read first camera
        readCamera1(fSettings, parameters);
        cout << "\t-Loaded camera 1" << endl;

        //Read second camera if stereo (not rectified)
        if(sensor_ == System::STEREO || sensor_ == System::IMU_STEREO){
            readCamera2(fSettings, parameters);
            cout << "\t-Loaded camera 2" << endl;
        }

        //Read image info
        readImageInfo(fSettings, parameters);
    ...
    void Settings::readCamera1(cv::FileStorage &fSettings, const CameraInfo &parameters) {
        bool found;
        ...
        if (cameraModel == "PinHole") {
            ...
            if (parameters.useCameraInfo)
            {
                ...
                vPinHoleDistorsion1_[0] = parameters.k1;
                vPinHoleDistorsion1_[1] = parameters.k2;
                vPinHoleDistorsion1_[2] = parameters.p1;
                vPinHoleDistorsion1_[3] = parameters.p2;
                vPinHoleDistorsion1_[4] = parameters.k3;
            }
            else
            {
                //Read intrinsic parameters
    ...
    void Settings::readCamera2(cv::FileStorage &fSettings, const CameraInfo &parameters) {
        bool found;
        vector<float> vCalibration;
    ...
    void Settings::readImageInfo(cv::FileStorage &fSettings, const CameraInfo &parameters) {
        bool found;
        int originalRows = 0;
        int originalCols = 0;
        ...
        if (parameters.useCameraInfo)
        {
            originalRows = parameters.height;
            originalCols = parameters.width; 
        }
        else
        {
            originalRows = readParameter<int>(fSettings,"Camera.height",found);
            originalCols = readParameter<int>(fSettings,"Camera.width",found);
        }
