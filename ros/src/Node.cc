...
void Node::Update()
{
  ...
  Sophus::SE3f position_se3f = orb_slam_->GetCurrentPosition();
  cv::Mat position = FromSE3fToMat(position_se3f);
  if (!position.empty()) {
...
cv::Mat Node::FromSE3fToMat(const Sophus::SE3f& position_mat)
{
  // Extract the rotation matrix and translation vector from Sophus::SE3f
  Eigen::Matrix3f rotation_matrix = position_mat.rotationMatrix();
  Eigen::Vector3f translation_vector = position_mat.translation();
  
  // Convert Eigen types to cv::Mat manually
  cv::Mat cv_rotation = cv::Mat(3, 3, CV_32F);
  cv::Mat cv_translation = cv::Mat(3, 1, CV_32F);

  for(int i=0; i<3; ++i)
  {
    cv_translation.at<float>(i,0) = translation_vector(i);
    for(int j=0; j<3; ++j)
    {
      cv_rotation.at<float>(i,j) = rotation_matrix(i,j);
    }
  }

  // Combine rotation and translation into a single cv::Mat
  cv::Mat cv_mat = cv::Mat::eye(4, 4, CV_32F);
  cv_rotation.copyTo(cv_mat(cv::Rect(0, 0, 3, 3)));
  cv_translation.copyTo(cv_mat(cv::Rect(3, 0, 1, 3)));
  ...
  std::vector<ORB_SLAM3::MapPoint *> map_points)
{
  ...
  for (unsigned int i = 0; i < cloud.width; i++) {
    ...
    if (map_points.at(i)->nObs >= min_observations_per_point_) {
      ...
      data_array[0] = map_points.at(i)->GetWorldPos()[2]; //.at<float>(2);
      // y. Do the transformation by just reading at the position of x instead of y
      data_array[1] = -1.0 * map_points.at(i)->GetWorldPos()[0]; //.at<float>(0);
      // z. Do the transformation by just reading at the position of y instead of z
      data_array[2] = -1.0 * map_points.at(i)->GetWorldPos()[1]; //.at<float>(1);
  ...
  const shared_ptr<orb_slam2_ros::srv::SaveMap::Response> response)
{
  if (request->name == "text")
      ...
      orb_slam_->SaveAtlas(ORB_SLAM3::System::FileType::BINARY_FILE);
...
{
  ORB_SLAM3::CameraInfo parameters;
  parameters.useCameraInfo = true;
  ...
  parameters.width = camera_info->width;
  parameters.height = camera_info->height; 

  orb_slam_ = new ORB_SLAM3::System(
      voc_file_name_param_,
      setting_file_name_param_, sensor_, true, 0, "",  parameters);
